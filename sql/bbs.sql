...
CREATE TABLE IF NOT EXISTS `bbs`.`awards` (
  ...
  `probability` DECIMAL(8,5) NULL DEFAULT NULL COMMENT '中奖概率',
  PRIMARY KEY (`id`))
ENGINE = InnoDB
...
CREATE TABLE IF NOT EXISTS `bbs`.`post` (
  ...
  `status` INT(1) NULL DEFAULT NULL COMMENT '帖子状态(0普通、1精华)',
  `top` INT(1) NULL DEFAULT NULL COMMENT '帖子置顶 1置顶 0不置顶',
  `type` INT(1) NULL DEFAULT NULL COMMENT '帖子类型(0讨论帖 1投票贴)',
  `expireTime` DATETIME NULL DEFAULT NULL COMMENT '投票贴结束时间',
  `is_delete` INT(1) NULL DEFAULT NULL COMMENT '是否删除(1是0否)',
  PRIMARY KEY (`id`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8
COMMENT = '帖子';
...
-- ----------------------------
-- Table structure for user_vote_post
-- ----------------------------
DROP TABLE IF EXISTS `user_vote_post`;
CREATE TABLE `user_vote_post` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT '主键id',
  `postId` int(11) DEFAULT NULL COMMENT '该用户投票的帖子id',
  `userId` int(11) DEFAULT NULL COMMENT '用户id',
  PRIMARY KEY (`id`)
...
INSERT INTO `user` (`id`,`name`,`openid`,`nick_name`,`avatar`) VALUES (1,'admin','21232f297a57a5a743894a0e4a801fc3','管理员',NULL);
